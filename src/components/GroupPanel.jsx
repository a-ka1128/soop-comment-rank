function rejectionReason(detection, postError, commentCount) {
  // 본문을 아예 못 읽은 경우. 댓글은 정상이므로 순위 자체는 믿을 수 있다.
  if (postError) {
    return `본문을 읽지 못해(${postError}) 분류 없이 좋아요 순으로만 나열합니다. 댓글과 순위는 정상입니다.`
  }
  // 댓글이 없으면 적중률이 0%로 나오는데, 그걸 "분류가 아닌 목록"이라고 하면 거짓말이다.
  if (commentCount === 0) return '댓글이 없어 나눌 것이 없습니다.'
 ...
 */
export default function GroupPanel({ groups, counts, detection, postError, commentCount }) {
  return (
      ...
      <div className="panel-head">
        ...
        <p>
          {groups.length > 0
            ...
            : rejectionReason(detection, postError, commentCount)}
        </p>
      </div>
